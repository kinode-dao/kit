    ...
    match message:
        ...
        case MessageRequest():
            ...
            if "Number" in body:
                ...
                print_to_terminal(
                    0,
                    f"fibonacci: fibonacci({number}) = {result}; {duration}ns",
                )
                send_response(
                ...
                print_to_terminal(
                    0,
                    f"fibonacci: fibonacci({number}) = {result}; {duration}±{absolute_deviation}ns averaged over {number_trials} trials",
                )
                send_response(
                    Response(
                        False,
...
class Process(process.Process):
    def init(self, our):
        print_to_terminal(0, "fibonacci: begin (python)")
        ...
        while True:
            try:
                ...
                print_to_terminal(0, f"fibonacci: error: {e}")
